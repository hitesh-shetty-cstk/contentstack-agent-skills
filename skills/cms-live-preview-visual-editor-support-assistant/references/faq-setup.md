...
- **root_cause**: The `live_preview` hash on the request is not one Contentstack issued for a live preview session. Contentstack generates the hash when the entry is opened in the preview pane and hands it to the site: as `?live_preview=<hash>` on the URL for SSR (and when the preview is opened in a new tab), or to the SDK running in the pane for CSR, where it surfaces as `ContentstackLivePreview.hash`. A hash from anywhere else is rejected with this error. The error means either (a) the app sent a hardcoded, stale, or hand-invented hash, (b) the app sent `live_preview=true` or an empty value instead of the real hash, or (c) the request was made outside any preview session (the site opened directly, or a custom preview tool that has no way to obtain a hash).
- **fix**:
  ...
  2. Confirm the SDK initialised: the Live Preview Onboarding Check must get past **Live Preview SDK Not Initialized**. That item clears only when the SDK's init message reaches the CMS, and the SDK sends it only when `init()` ran on the client with a truthy `enable`. If it sticks there, check `enable`, the `.env` value actually parsing as `true`, and that `init()` runs on the client.
...
- **verification**: With the entry open in Live Preview, the site's content requests go to the preview host and return 200, and the 382 error is gone.
...
- **root_cause**: `ContentstackLivePreview.init()` has to run in the browser. In App Router, code placed in a server component or in the root layout module body never executes client-side, so the SDK never installs its postMessage listeners and the Onboarding Check stays on **Live Preview SDK Not Initialized**.
- **fix**:
...
- **fix**:
  ...
  5. If the card flashes then clears on a working site, no action is needed — the first preview request was simply slow to land.
